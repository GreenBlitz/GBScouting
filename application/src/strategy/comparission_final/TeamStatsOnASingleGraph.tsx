...
const TeamStatsOnASingleGraph: React.FC = ()=>{
    ...
    const graph = (independentVariable: independentVariable) => <LineChart height={400} width={400} dataSets={
        {Speaker1: ["blue", new TeamData(matches1).getAsLine(independentVariable)], Speaker2: ["red", new TeamData(matches1).getAsLine(independentVariable)]}}></LineChart>
    ...
    return(
        <>
            <select id="teams1" onChange={handleSelect1}>{FRCTeamList.map(teamName => (
                <option value={teamName}>{teamName}</option>
            ))}</select>
            <select id="teams2" onChange={handleSelect2}>{FRCTeamList.map(teamName => (
                <option value={teamName}>{teamName}</option>
            ))}</select>
