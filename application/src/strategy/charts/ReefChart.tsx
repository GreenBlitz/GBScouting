...
const ReefChart: React.FC<{ sides: ReefSide[] }> = ({ sides }) => {
  ...
  function drawAll() {
    ...
    if (canvasContext) {
      ...
      drawButtons(canvasContext);
    }
  }

  function drawButtons(context: CanvasRenderingContext2D) {
    const middleOfHexagon = { x: width / 2, y: height / 2 };
    hexagonVertices.forEach((vertex, index) => {
      context.beginPath();
      const isUsed =
        sides.filter((side) =>
          areReefsSame(triangleButtonMiddles[index].reefSide, side)
        ).length > 0;
      context.fillStyle = isUsed ? "#18723c" : "red";
      const nextVertex =
