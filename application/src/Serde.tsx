...
function serdeUnsignedNum(bitCount: number) {
  const bits = bitCount;
...
function serdeArray(itemSerde: ArrayItemsSerde, bitCount = ARRAY_LENGTH_DEFAULT_BIT_COUNT) {
  ...
  function deserializer(
    ...
    for (let i = 0; i < arrayLength; i++) {
      let item: any = itemDeserializer(serializedData);
      arr.push(item);
    }
    return arr;
  }
...
function serdeEnumedString(possibleValues: string[]) {
  ...
  return {
    ...
    bitsNeeded: function() {
      return bitsNeeded(possibleValues.length);
    },
  };
}

function serdeStringifiedArray(itemSerde: ArrayItemsSerde) {
  function serializer(itemSerde: ArrayItemsSerde, serializedData: BitArray, arr) {
    if (arr == undefined) {
      console.log("TRYING TO PARSE undefined");
    }
    serdeArray(itemSerde).serializer(serializedData, JSON.parse(arr));
  }
  function deserializer(itemSerde: ArrayItemsSerde, serializedData) {
...
function serdeBool() {
  function serializer(serializedData, bool: boolean) {
...
function serdeRecordFieldsBuilder(fieldNamesSerdes: [string, ArrayItemsSerde][]): RecordFieldsSerde {
...
export const qrSerde: RecordFieldsSerde = serdeRecordFieldsBuilder([
