...
class TeleopForm extends ScouterInput<Levels & NetScore & Algea, {}, { levels: Levels; undoStack: FullAction[]; netScore: NetScore, algeaScore: Algea }> {
  ...
  getStartingState(props: InputProps<Levels>): { levels: Levels; undoStack: FullAction[], netScore: NetScore, algeaScore: Algea } {
    return {
  ...
  renderInput(): React.ReactNode {
    ...
    const handleClick = (action: CoralAction) => {
      ...
      this.storage.set({...updatedLevels, score: this.state.netScore.score, miss: this.state.netScore.miss, ...this.state.algeaScore});
    };
    ...
    const handleNet = (action: NetAction) => {
        ...
        this.storage.set({...this.state.levels, ...updatedNetScore, ...this.state.algeaScore});
      };
      ...
      const handleAlgea = (action: AlgeaAction) => {
        ...
        this.storage.set({...this.state.levels, ...this.state.netScore, ...updatedAlgeaScore});
      ...
      const handleUndo = () => {
        ...
        } else if ("type" in lastAction && lastAction.type === "score" || lastAction.type === "miss") {
        ...
        this.storage.set({ ...updatedLevels, ...updatedNetScore, ...updatedAlgea });
      };
  ...
  <div className="container">
    ...
    <div className="section">
      <button className="buttonU">Back</button>
      <button className="buttonU">Next</button>
    </div>
  </div>
);
